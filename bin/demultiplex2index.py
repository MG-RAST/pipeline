...
def barcode_files(bfile, odir, stype, prefix, rc_bar):
    ...
    mapping_index_file = bfile + ".subset"
    ...
    barcode2index = defaultdict(list)
    ...
    for bn in bar_name:
        ...
        barcode2index[barc].append(name + ".subset")
        barcode2index[barc].append(mapping_index_file)
        #  barcode2mapping_file[barc] = ...
      
        uniq_fname[name] = None
        uniq_fname[name + ".subset"] = None
        uniq_fname[mapping_index_file] = None
    return barcode2index , barc_fname, uniq_fname
...
def main(args):
    ...
    barcode2index , barc_fname, uniq_fname = barcode_files(opts.barcode, opts.output, opts.format, opts.prefix, opts.rc_bar)
    # open filehandles
    for f in uniq_fname :
        print(f)
    ...
    miss_count = 0
    input_hdl  = open(opts.input, 'r')
    index_hdl  = open(opts.input + ".record.idx", 'w')
    record_counter = 0 
    ...
    for rec in seq_iter(input_hdl, opts.format):
        ...
        while line_nr < 10 :
            ...
            if (qual + "\n") == line :
                ...
                pass
        print(record_counter , offset , pointer.tell() , str( pointer.tell() - offset )  )
        index_hdl.write( str(offset) + "\t" + str( pointer.tell() - offset ) + "\n" ) 
        offset = pointer.tell()
        ...
        if match:
            ...
            for f in barcode2index[mbar]:
                uniq_fname[f].write( "%d\n" %(record_counter)  )
        else:
            miss_count += 1
    ...
    #close filehandles
    missing.close()
    for k, fhdl in uniq_fname.items():
        fhdl.close()

    if opts.verbose:
        print(
            "%d sequences split amoung %d barcodes. %d sequences without barcodes"%(sum(bar_count.values()), len(bar_count.keys()), miss_count)
        )
