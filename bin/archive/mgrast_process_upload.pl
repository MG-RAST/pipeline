...
my $lock_file   = "$base_dir/mgrast_process_upload.lock";
...
{
    $ts = &timestamp;
    print "[$ts] lock file '$lock_file' found, assuming earlier process is still running and exiting\n";
    exit;
}
else
{
    # don't forget to delete lock file when done
    open(LOCK, ">$lock_file") or die "could not open file '$lock_file': $!";
    close(LOCK) or die "could not close file '$lock_file': $!";
    chmod 0666, $lock_file;
}

my $upload_status = &upload_status($status_file);

foreach my $rec ( @{ $upload_status->{demultiplex} } )
{
    # perform demultiplexing
    # update status to demultiplexing completed in status file
    # add demultiplexed files to has upload_status with 'upload_completed' status so that they get processed like other uploads
    
    my($user_dir, $upload_dir, $upload_filename) = @$rec;
...
foreach my $rec ( @{ $upload_status->{upload_completed} } )
{
    # these do not contain complete path information -- need to straighten this out
    my($user_dir, $upload_dir, $upload_filename) = @$rec;
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] starting on $upload_dir, $upload_filename\n";
    }

    my $user_upload_dir = "$base_dir/$user_dir/$upload_dir";

    if ( ! -d $user_upload_dir )
    {
	die "directory '$user_upload_dir' does not exist\n";
    }
    
    if ( ! -e "$user_upload_dir/$upload_filename" )
    {
	die "uploaded file '$user_upload_dir/$upload_filename' does not exist\n";
    }

    if ( ! -s "$user_upload_dir/$upload_filename" ) 
    {
	die "uploaded file '$user_upload_dir/$upload_filename' has zero size";
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] starting unpacking $upload_dir, $upload_filename\n";
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] completed unpacking $upload_dir, $upload_filename\n";
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] starting file tests, $upload_filename\n";
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] finished file tests, $upload_filename\n";
	print "[$ts] starting file_seq_type test, $upload_filename\n";
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] finished file_seq_type test, $upload_filename\n";
    }
	...
	if ( $verbose ) {
	    $ts = &timestamp;
	    print "[$ts] getting fasta from fastq, $upload_filename\n";
	}
	...
	if ( $verbose ) {
	    $ts = &timestamp;
	    print "[$ts] done getting fasta from fastq, $upload_filename\n";
	}
    }
	...
	if ( $verbose ) {
	    $ts = &timestamp;
	    print "[$ts] getting fasta from sff, $upload_filename\n";
	}
	...
	{
	    # log error
	    die "could not extract fasta sequence file from sff file '$sff_file'";
	}
	
	if ( $verbose ) {
	    $ts = &timestamp;
	    print "[$ts] done getting fasta from sff, $upload_filename\n" if $verbose;
	}
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] getting fasta stats, $upload_filename\n";
    }
    ...
    if ( $verbose ) {
	$ts = &timestamp;
	print "[$ts] done getting fasta stats, $upload_filename\n";
    }
...
unlink $lock_file or die "could not delete lock file '$lock_file'\n";
...
sub file_type {
    ...
    open(P, "-|", "file", "$dir/$file") or die("cannot run file command on file '$dir/$file': $!");
    my $fc = <P>;
    chomp $fc;
    close(P);

    my $file_type = ( split(':', $fc) )[1];
