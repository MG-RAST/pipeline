...
sub fasta_report_and_stats {
    ...
    ### report keys:
    # bp_count, sequence_count, average_length, standard_deviation_length, length_max, length_min,
    # average_gc_content, standard_deviation_gc_content, average_gc_ratio, standard_deviation_gc_ratio,
    # ambig_char_count, ambig_sequence_count, average_ambig_chars, file_size, sequencing_method_guess, sequence_type
    ...
    my @stats = `seq_length_stats.py -i '$file_path/$file_name' -s 2>&1`;
    chomp @stats;
    ...
    my $header = `head -1 $file_path/$file_name`;
    chomp $header;
    ...
    $report->{sequencing_method_guess} = $method;
...
sub gunzip_file {
	...
	# untar on a gzipped file can produce junk files which should be ignored
	@tar_files = ();
